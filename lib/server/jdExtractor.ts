...
function buildPrompt(roleTitle: string, jdText: string): string {
  return [
    ...
    'For must_haves: prioritize concrete technical skills/tools/frameworks/platforms explicitly present in JD.',
    'must_haves should generally contain 5-8 items. Avoid generic soft skills unless clearly required; max 1 generic soft skill.',
    'Preserve important multi-word terms as-is when possible (e.g., Retrieval-Augmented Generation, Vector Databases).',
    'Put leadership/communication/problem-solving in nice_to_haves unless they are explicit hard requirements.',
    'Use preferred/optional sections to populate nice_to_haves.',
