...
export default function Page() {
  ...
  const [requestedEditSetupId, setRequestedEditSetupId] = useState('');
  ...
  useEffect(() => {
    ...
    setActiveTab(tab);
    setShowInterviewSetup(tab === 'interviews' && q.get('setup') === '1');
    setRequestedEditSetupId(q.get('editSetupId') || '');
  ...
  const startCreateInterview = () => {
    ...
    setSuccess('');
    setError('');
    router.push('/?tab=interviews&setup=1');
  };

  const startEditInterview = (interview: InterviewRecord, skipNavigation = false) => {
    ...
    if (!skipNavigation) {
      router.push(`/?tab=interviews&setup=1&editSetupId=${encodeURIComponent(interview.id)}`);
    }
  };

  useEffect(() => {
    if (!requestedEditSetupId || loading) return;
    const interview = interviews.find((item) => item.id === requestedEditSetupId);
    if (interview) {
      startEditInterview(interview, true);
    }
    setRequestedEditSetupId('');
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [requestedEditSetupId, loading, interviews, positions]);
                ...
                {todayInterviews.map((item) => (
                  <div key={item.id} className={styles.interviewCard}>
                    ...
                    <div className={styles.cardButtons}>
                      ...
                      {String(item.transcriptText || '').trim() ? (
                        <a
                          className="lk-button"
                  ...
                  {positions.map((position) => (
                    <div key={position.position_id} className={styles.reportPanel}>
                      ...
                      <p className={styles.interviewMeta}>
                        {`Must-haves: ${position.must_haves.join(', ') || 'None'} | Nice-to-haves: ${position.nice_to_haves.join(', ') || 'None'
                          }`}
                      </p>
                    ...
                    .map((candidate) => (
                      <div key={candidate.id} className={styles.reportPanel}>
                        ...
                        {candidateScreeningById[candidate.id] ? (
                          <p className={styles.interviewMeta}>
                            {`Screening: ${candidateScreeningById[candidate.id]?.blendedRecommendation?.toUpperCase() ||
                              candidateScreeningById[candidate.id]?.recommendation.toUpperCase()
                              } | Blended ${candidateScreeningById[candidate.id]?.blendedScore ??
                    ...
                    return (
                      <div key={application.id} className={styles.reportPanel}>
                        ...
                        <div className={styles.cardButtons}>
                          ...
                          {latestInterview && String(latestInterview.transcriptText || '').trim() ? (
                            <a
                              className="lk-button"
